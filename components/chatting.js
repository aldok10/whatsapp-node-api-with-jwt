...
router.post('/text/:phone', async (req,res) => {
    ...
    if(phone==undefined||message==undefined){
        res.send({status: false, message: "Tolong Masukkan No HP & Pesan Dengan Benar"})
    } else {
        client.sendMessage(phone+'@c.us',message).then((response)=>{
            if(response.id.fromMe){
                res.send({
                    status: true,
                    message: 'Pesan berhasil dikirim ke nomor '+phone,
                    data : response
                })
            }
        });
...
router.post('/image/:phone', async (req,res) => {
...
router.post('/pdf/:phone', async (req,res) => {
    var base64regex = /^([0-9a-zA-Z+/]{4})*(([0-9a-zA-Z+/]{2}==)|([0-9a-zA-Z+/]{3}=))?$/;
...
router.post('/location/:phone', async (req,res) => {
    let phone = req.params.phone;
